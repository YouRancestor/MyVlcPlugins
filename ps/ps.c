...
vlc_module_begin ()
    set_description( N_("My PS muxer") )
    set_shortname( "My MPEG-PS" )
    set_category( CAT_SOUT )
    set_subcategory( SUBCAT_SOUT_MUX )
    set_capability( "sout mux", 51 )
...
static int Mux( sout_mux_t *p_mux )
{
    ...
    for( ;; )
    {
        ...
        /*保存文件*/
        FILE* pFile =NULL;
        pFile = fopen("C:\\Users\\zYr\\Desktop\\testfiles\\send.ps","ab+");
        if(pFile)
            fwrite(p_ps->p_buffer,1,p_ps->i_buffer,pFile);
        fclose(pFile);
