...
vlc_module_begin ()
    set_shortname( "My HEVC")
    set_category( CAT_INPUT )
    set_subcategory( SUBCAT_INPUT_DEMUX )
    set_description( N_("My HEVC/H.265 video demuxer" ) )
...
//====================
    ...
    int i_width =0;
    int i_height =0;
    //getWH(p_demux->psz_file,&i_width,&i_height);
    getWH_SPS(p_demux->psz_file,&i_width,&i_height);
    p_sys->p_packetizer->fmt_out.b_packetized = true;
    p_sys->p_packetizer->fmt_out.video.i_width=i_width;
    p_sys->p_packetizer->fmt_out.video.i_height=i_height;
