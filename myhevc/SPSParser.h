...
enum SPSPARSESTATUS     // NO_SPS_FOUND means need more data; NO_NAL_FOUND means the input data is not nal data; BAD_DATA means the SPS data has beed found,but it is bad.
{
    PARSE_SUCCESS = 0,
    NO_SPS_FOUND = 1,
    NO_NAL_FOUND = 2,
...
public:
    SPSParser();
    ~SPSParser();
    SPSPARSESTATUS readSPS(NAL* pFirstNal);
    SPSPARSESTATUS parseSPS(char* data, int size, bool is265);
    void getSize(int& width, int& height);
private:
    bool                    m_is265;
    h264_stream_t*          m_h;
    vc_params_t*            m_pParams;
    bool                    parseSequenceParameterSet(unsigned char* data,int size, vc_params_t& params);
    bool                    read_nal_unit(unsigned char* buf, unsigned int size);
    ...
    bs_t*                   bs_new(unsigned char* buf, int size);
    void                    bs_free(bs_t* b);
...
public:
    ...
    NAL( const NAL &nal )
    {
        length = nal.length;
        curLen = nal.curLen;
        bitstream = new unsigned char [length];
        curPos = bitstream;
        memcpy( bitstream, nal.bitstream, length*sizeof(unsigned char) );
    }

    ~NAL()
    {
        if(bitstream)
        {	delete[] bitstream; bitstream = NULL; }
        curPos = NULL;
    }

    NAL &operator= ( const NAL &nal )
    {
        length = nal.length;
        curLen = nal.curLen;
        bitstream = new unsigned char [length];
...
//void nal_to_rbsp(unsigned char* nal_buf, int* nal_size, unsigned char* rbsp_buf, int* rbsp_size);
...
public:
    ...
    void Init(void * data, int len) { m_data = (unsigned char*)data; m_len = len; m_idx = 0; m_bits = 0; m_byte = 0; m_zeros = 0; };
