...
//    }
    ...
    switch( query->i_type )
    {
        ...
        {   /* Aggregate-only */
            ...
            if(psz_urlArgs != NULL)
            {
                //creat new url,just like a session @@##
                uint64_t   tmp_id;
                vlc_rand_bytes (&tmp_id, sizeof (tmp_id));
                snprintf( psz_sesbuf, sizeof( psz_sesbuf ), "%"PRIx64,
                          tmp_id );
                psz_session = psz_sesbuf;

                strcpy(control,"rtsp://127.0.0.1:8556/");
                strcat(control,psz_session);

                //create vlm_media @@##

                vlm_media_t cfg;
                vlm_media_t * p_cfg = &cfg;
                int64_t id;

                vlm_media_Init( &cfg );
                cfg.psz_name = strdup( psz_session );//psz_name
                cfg.b_vod = 1;//!strcmp( psz_type, "vod" );
                cfg.b_enabled = true;
                char psz_value[100] ;
                strcpy(psz_value,"xiamu.flv");
                TAB_APPEND( p_cfg->i_input, p_cfg->ppsz_input, strdup(psz_value) );
                if( vlm_Control( p_vlm, VLM_ADD_MEDIA, &cfg, &id ) )
                {
                    vlm_media_Clean( &cfg );
            ...
            if(psz_urlArgs != NULL)
            {
                answer->p_body = (uint8_t *) ( vod ?
                    SDPGenerateVoD2( rtsp->vod_media,control,psz_session ) : //@@##
...
//                        s1.channel = i_interleaved;
        ...
        case HTTPD_MSG_PLAY:
        {
            ...
            if (range != NULL)
            {
                ...
                //vlm_media_sys_t *p_media = vlm_ControlMediaGetById( p_vlm, ses->vlm_media_id );//p_media->vod.p_media
